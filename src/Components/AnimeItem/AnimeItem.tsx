...
function AnimeItem() {
    ...
    const {likeBids} = useSelector((state: RootState) => state.MainSlice.profile);
    const profile = useSelector((state: RootState) => state.MainSlice.profile);

    const [streamingLink, setStreamingLink] = useState('');
    const [episode, setEpisode] = useState(1);
    // const [watchStatusModal, changeWatchStatus] = useState(false);

    const dispatch = useDispatch();

    const getEpisodes = async (id: string) => {
        const url = "https://api.consumet.org/anime/gogoanime/watch/" + id + `-episode-${episode}`;
        try {
            const { data } = await axios.get(url, { params: { server: "gogocdn" } });
            setStreamingLink(data.sources[data.sources.length - 3].url)
        } catch (err) {
        }
    };
    ...
    return (
        <>
            {/*<div className={styles.watchModal}>*/}
            ...
            <div className={styles.animeItem}>
                <div className={styles.animeInfo}>
                    ...
                    <div style={profileCardOpen ? {maxWidth: '820px'} : {maxWidth: '1200px'}} className={styles.description}>
                        <div className={styles.content}>
                            <h3 className={styles.title}>{currentAnime.title}<br/><span>({currentAnime.otherName})</span></h3>
                            <p className={styles.subtitle}>{currentAnime.description}</p>
                            <div className={styles.genres}>
                                {currentAnime.genres?.map((item, i) => {
                                    return <div className={styles.genreItem} key={item + i}>{item}</div>
                                })}
                            </div>
                            <p className={styles.episodesCount}>Episodes: {currentAnime.totalEpisodes}</p>
                            <p className={styles.status}>Status: {currentAnime.status}</p>
                        </div>
                        <button className={styles.watchedStatus}>
                            Watched
                            <svg xmlns="http://www.w3.org/2000/svg" version="1.1" x="0" y="0" viewBox="0 0 100 100"><g><path d="M79.41 65.41c-.39.39-.9.59-1.41.59s-1.02-.2-1.41-.59L50 38.83 23.41 65.41c-.78.78-2.05.78-2.83 0s-.78-2.05 0-2.83l28-28c.78-.78 2.05-.78 2.83 0l28 28c.79.79.79 2.05 0 2.83z" fill="#fff" data-original="#000000"></path></g></svg>
                        </button>
                    </div>
                    ...
                    <div className={styles.buttonLike}>
                        {
                            !!Object.keys(profile).length ?
                                likeBids?.some(item => item === currentAnime.id) ?
                                    <button className={styles.like} onClick={() => unlike(currentAnime.id? currentAnime.id : '1')}>
                                    ...
                                    :
                                    <button className={styles.unlike} onClick={() => like(currentAnime.id? currentAnime.id : '1')}>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="22" height="20" viewBox="0 0 22 20" fill="none">
                ...
                <div className={styles.playerWrap}>
                    <ReactHlsPlayer
                        ...
                        controls={true}
                        width="100%"
                        src={'https://cors.dekianime.site/' + streamingLink}
                    />
                    <div className={styles.episodes}>
                        <p className={styles.episodesTitle}>Episodes</p>
                        {currentAnime.episodes?.map(item => {
                            return (
                                <button className={episode === item.number ? `${styles.episode} ${styles.active}` : styles.episode} onClick={() => setEpisode(item.number)} key={item.id}>{item.number}</button>
                            )
                        })}
                    </div>
                </div>
