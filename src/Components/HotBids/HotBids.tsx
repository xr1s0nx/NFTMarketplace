...
import {bidsTimerStart, likeBid, toggleSingModalOpenStatus, unlikeBid} from "../../Redux/Slices/MainSlice";
import {doc, updateDoc} from "firebase/firestore";
import {db} from "../../firebase";

function HotBids() {

    const hotBids = useSelector((state: RootState) => state.MainSlice.hotBids);
    ...
    return (
        <div className={styles.hotBids}>
            <h2 className={styles.title}>🔥 Hot Bids</h2>
            <div className={styles.bids}>
                {hotBids ?
                    hotBids.map(item => {
                        return (
                            <div key={item.id} className={styles.bid}>
                                <div className={styles.top}>
                                    <div className={styles.timerBlock}>
                                        <span className={styles.bg}>
                                            <img src={timerBg} alt=""/>
                                        </span>
                                        <p className={styles.timer}>{item.time.hours < 10 ? `0${item.time.hours}` : item.time.hours}:{item.time.minutes < 10 ? `0${item.time.minutes}` : item.time.minutes}:{item.time.seconds < 10 ? `0${item.time.seconds}` : item.time.seconds}</p>
                                    </div>
                                        ...
                                        <p className={styles.likes}>{item.likes}</p>
                                        {
                                            !!Object.keys(profile).length ?
                                                ...
                                                profile.likeBids.some(id => id === item.id) ?
                                                    <button className={styles.like} onClick={() => unlike(item.id)}>
                                                    ...
                                                    :
                                                    <button className={styles.unlike} onClick={() => like(item.id)}>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="22" height="20" viewBox="0 0 22 20" fill="none">
                                ...
                                <NavLink className={styles.bidImage} to={'/bids/' + item.id}>
                                    <img src={item.imageUrl} alt=""/>
                                </NavLink>
                                <p className={styles.bidTitle}>{item.title}</p>
                                <div className={styles.bottom}>
                                    <p className={styles.price}>{item.price} ETH</p>
                                    <div className={styles.topBuyers}>
                                        {item.topBuyers ? item.topBuyers.map(buyer => {
                                            return (
                                                <div key={buyer.id} className={styles.buyer}>
                                                    <img src={buyer.avatarUrl ? buyer.avatarUrl : defaultAvatar} alt=""/>
                                                </div>
                                            )
                                        }) : null}
                                    </div>
